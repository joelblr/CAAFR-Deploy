...
class AzonScraperFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        heading_lbl = ctk.CTkLabel(self, text="Web Scraper Tool: Amazon", font=("Georgia bold", 18), text_color="white")
    ...
    def scrape_action(self):
        ...
        if (not catg_menu or catg_menu == "Select Category") and new_catg == "":
            ...
            logics.log_error("fas_err.log", f"frame-azon-scraper.py", errHead, errBody)
        ...
        if not save_as:
            save_as = ("final" if scrape_mode=="Deploy" else "test") + "_azon_" + catg_name
        ...
        threading.Thread(target=self.run_in_background, args=(
            ...
            os.path.join("..", "..", "server", "azon_scraper.js"),
            data,
        )).start()
    ...
    def update_menu_option(self, scrapeType):
        dbDir1 = os.path.join(os.path.dirname(__file__), "..", "..", "..", "database", "azon")
        dbDir2 = os.path.join(os.path.dirname(__file__), "..", "..", "database", "azon")
